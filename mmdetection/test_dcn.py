...
def train(config):
    ...
    if "val_json" in config:
        ...
        name_list =  config['val_json'].split('.')
        name_list[0] += '_'
        
        cfg.data.val.ann_file = root +'classwise_dataset/'+name_list[0]  # test json 정보
        #cfg.data.train.pipeline[3]['policies'][0][0]['img_scale'] = tuple(config['resize']) # Resize
    ...
    backup_cfg = cfg.data.val.copy()
    datasets=[]
    for idx in range(10):
        next_cfg = backup_cfg.copy()
        next_cfg.ann_file+=f'{idx}.json'
        cfg.data.val = next_cfg
        print(next_cfg.ann_file)
        print(next_cfg)
        datasets.append(build_dataset(cfg.data.val))
    cfg.workflow=[('val',1)]*10
    print(len(datasets))
    print(len(cfg.workflow))
